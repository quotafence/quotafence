...
| Managed interactive launch | Implemented through `qfence codex` | Implemented through `qfence claude` against the Weekly workspace allocation | One managed reservation tracks the Weekly project budget while every active native window remains a provider safety limit |
